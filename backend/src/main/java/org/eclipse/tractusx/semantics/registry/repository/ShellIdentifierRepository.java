...
public interface ShellIdentifierRepository extends JpaRepository<ShellIdentifier, UUID> {
   ...
   @Query( value = """
            ...
            WHERE
                ...
                AND NOT (:tenantId = :owningTenantId) -- Avoid duplicates if the first condition also matches
            GROUP BY s.id_external, s.created_date
            HAVING COUNT(*) = :keyValueCombinationsSize
            ...
            WHERE
                ...
                AND EXISTS (
                    ...
                    WHERE
                        ...
                        AND (sider.ref_key_value = :tenantId OR (sider.ref_key_value = :publicWildcardPrefix AND si.namespace IN (:publicWildcardAllowedTypes)))
                )
                AND NOT (:tenantId = :owningTenantId OR si.namespace = :globalAssetId) -- Avoid duplicates from previous unions
            GROUP BY s.id_external, s.created_date
            HAVING COUNT(*) = :keyValueCombinationsSize
